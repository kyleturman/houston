class UpdateThreadMessagesToPolymorphicAgentable < ActiveRecord::Migration[8.0]
  def change
    # Add polymorphic columns for agentable
    add_column :thread_messages, :agentable_type, :string
    add_column :thread_messages, :agentable_id, :bigint

    # Add index for polymorphic association
    add_index :thread_messages, [:agentable_type, :agentable_id]
    ...
    reversible do |dir|
      dir.up do
        ...
        execute <<-SQL
          ...
          WHERE thread_messages.agent_instance_id = ai.id 
    ...
    change_column_null :thread_messages, :agentable_type, false
    change_column_null :thread_messages, :agentable_id, false
  end
end
