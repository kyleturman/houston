...
function getConnections(): GoogleConnection[] {
...
function filterConnectionsByEmail(connections: GoogleConnection[], email?: string): GoogleConnection[] {
...
function ensureConnections(): GoogleConnection[] {
  const connections = getConnections();
  if (connections.length === 0) {
    throw new McpError(
      ErrorCode.InvalidRequest,
      'No Google accounts connected. Please connect a Google account first.'
    );
  }
  return connections;
}
  ...
  try {
    ...
    switch (name) {
      ...
      case 'calendar_update_event': {
        ...
        let foundInAccount: string | null = null;
      ...
      case 'calendar_delete_event': {
        ...
        let deleted = false;
        let deletedFromAccount: string | null = null;
      ...
      case 'calendar_free_busy': {
        ...
        const allResults: FreeBusyResult[] = [];
    ...
    if (errorMessage.includes('invalid_grant') || errorMessage.includes('Token has been expired')) {
      throw new McpError(
        ErrorCode.InvalidRequest,
        'Google access token has expired. Please reconnect the account.'
      );
    }
    ...
    if (errorMessage.includes('insufficient permission') || errorMessage.includes('403')) {
      throw new McpError(
        ErrorCode.InvalidRequest,
        'Insufficient Calendar permissions. Please reconnect with the required scopes.'
      );
    }

    throw new McpError(
      ErrorCode.InternalError,
      `Calendar operation failed: ${errorMessage}`
    );
  }
});
