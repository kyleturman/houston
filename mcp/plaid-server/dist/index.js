...
function filterConnectionsByInstitution(connections, institutionName) {
    if (!institutionName)
        return connections;
...
function ensureConnections() {
    ...
    if (connections.length === 0) {
        throw new McpError(ErrorCode.InvalidRequest, 'No Plaid connections available. Please connect a bank account first.');
    }
    return connections;
}
        ...
        switch (name) {
            ...
            case 'plaid_analyze_spending': {
                ...
                if (params.categories && params.categories.length > 0) {
                    filteredTransactions = allTransactions.filter(txn => txn.category?.some((cat) => params.categories.includes(cat)));
                }
                // Analyze spending
                ...
                const dayCount = Math.max(1, Math.ceil((new Date(params.endDate).getTime() - new Date(params.startDate).getTime()) / (1000 * 60 * 60 * 24)));
                return {
                    content: [{
                            type: 'text',
                            text: JSON.stringify({
            ...
            case 'plaid_get_recurring_transactions': {
                ...
                let connections = ensureConnections();
                connections = filterConnectionsByInstitution(connections, params.institutionName);
                const allInflowStreams = [];
                const allOutflowStreams = [];
                for (const conn of connections) {
                    try {
                        const request = {
                            access_token: conn.access_token,
